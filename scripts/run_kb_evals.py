...
# R3 isolation: MUST be set before any cora module import so every gate that
# reads it at call time (tools_for_entity, dispatch, gap_detection) is armed
...
def run_l1_case(case: dict) -> tuple[bool, str]:
    ...
    expect = _norm(case.get("expect_substring") or "")
    if not expect:
...
def summarize(results: list[dict], prev_failing: set[str]) -> dict:
    ...
    return {
        ...
        "newly_failing": sorted(failing_ids - prev_failing),
        "fixed_since_last": sorted(prev_failing - failing_ids),
        "failing_ids": sorted(failing_ids),
        "skipped_l2_only": sum(1 for r in results if r["ok"] is None),
    }
...
def main() -> int:
    ...
    summary = summarize(results, prev_failing)
    if not args.only_id:

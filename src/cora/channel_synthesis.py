...
# ---------------------------------------------------------------------------
# Founder-OS durable copy of the PORTFOLIO synthesis (write-at-generation)
# ---------------------------------------------------------------------------
# Retires the LLM-cost Cowork harvester (`fndr-daily-synthesis-persist`): Cora
# already holds the exact synthesized body, so it writes the durable Founder-OS
# markdown itself, deterministically, at generation time. The file matches the
# harvester's shape (title + provenance preamble + `---` + `##`-sectioned body +
# `---` + footer) so static_md ingest + downstream consumers see no seam. The
# harvester re-read the Slack post and re-formatted it with an LLM (its cost);
# Cora's copy is MORE faithful (it is the source, not a re-read). PORTFOLIO ONLY
# (the 8 entity syntheses are unchanged, matching today's harvester scope).

_DAILY_SYNTHESIS_TITLE = "Daily Portfolio Synthesis"
_DAILY_SYNTHESIS_PREAMBLE = (
    "_Generated by Cora at synthesis time from the #founder-operations daily "
    "portfolio briefing; written to Founder OS (durable per "
    "`_shared/playbooks/knowledge-persistence-doctrine.md`)._"
)
# A whole (stripped) line that is a single Slack-bold token -> a `##` section
# header. Slack section headers ride on their own line in the synthesis output;
# an inline `*x*` elsewhere becomes markdown `**x**` (see _slack_body_to_markdown).
_SLACK_SECTION_RE = re.compile(r"^\*([^*]+)\*$")
_INLINE_BOLD_RE = re.compile(r"\*([^*\n]+?)\*")


def _founder_os_root() -> Path:
    """Founder-OS root (env-overridable, same key session_capture uses)."""
    env = os.environ.get("FOUNDER_OS_ROOT", "").strip()
    return Path(env) if env else Path(r"G:\My Drive\HJR-Founder-OS")


def _daily_synthesis_path(today: date, *, root: Path | None = None) -> Path:
    """Founder-OS path for today's persisted portfolio synthesis. Matches the
    Cowork harvester convention EXACTLY: 00-Founder/_daily-synthesis/YYYY-MM/
    YYYY-MM-DD_fndr_daily-synthesis.md (the goal's flat YYYY-MM-DD.md was a
    simplification; the live files are month-foldered -- match reality)."""
    base = root or _founder_os_root()
    return (base / "00-Founder" / "_daily-synthesis" / today.strftime("%Y-%m")
            / f"{today.isoformat()}_fndr_daily-synthesis.md")


def _slack_body_to_markdown(body: str, today: date) -> str:
    ...
    for raw in text.split("\n"):
        m = _SLACK_SECTION_RE.match(raw.strip())
        if m:
...
def render_daily_synthesis_md(body: str, today: date) -> str:
    """Render the full Founder-OS markdown file for the portfolio synthesis.

    PURE (no I/O), so a dry-run can preview byte-similarity to the harvester
    format without writing. Structure: `# Title -- Weekday, ISO` / provenance
    preamble / `---` / `##`-sectioned body / `---` / source footer.
    """
    title = f"# {_DAILY_SYNTHESIS_TITLE} — {today.strftime('%A')}, {today.isoformat()}"
    md_body = _slack_body_to_markdown(body, today)
    footer = (
        "_Source: Cora daily portfolio synthesis, #founder-operations. "
        f"Written {today.isoformat()} by channel_synthesis (write-at-generation)._"
    )
    return (f"{title}\n\n{_DAILY_SYNTHESIS_PREAMBLE}\n\n---\n\n"
            f"{md_body}\n\n---\n\n{footer}\n")


def persist_daily_synthesis(md: str, today: date, *,
                            root: Path | None = None) -> Path | None:
    """Write the rendered markdown to Founder OS via drive_io (D-083 -- no naked
    G: write, bounded worker). FAIL-SOFT: any error (incl. drive_io.DriveUnavailable
    on a mount blip) logs WARN and returns None -- NEVER raises. The Slack post has
    already happened by the time this runs, so a failed write can never affect it."""
    try:
        from . import drive_io
        path = _daily_synthesis_path(today, root=root)
        drive_io.write_text_atomic(path, md, encoding="utf-8")
        log.info("channel_synthesis: persisted daily synthesis -> %s", path)
        return path
    except Exception as exc:  # noqa: BLE001 -- fail-soft; never raise
        log.warning("channel_synthesis: daily-synthesis persist failed: %s", exc)
        return None


def run_portfolio(*, dry_run: bool = False, today: date | None = None,
                  channel: str | None = None) -> dict:
    """Portfolio synthesis -> #founder-operations (holdco; covers HJRG), plus a
    durable Founder-OS markdown copy written at generation (Slice 1)."""
    today = today or _today()
    ...
    # Durable Founder-OS copy. The Slack post already fired inside run_synthesis,
    # and persist_daily_synthesis is fail-soft, so this step can NEVER block or
    # fail delivery. Rendered in BOTH modes so a dry-run previews the exact bytes;
    # only actually written when not dry_run (write-nothing dry-run invariant).
    try:
        md = render_daily_synthesis_md(result["body"], today)
        result["founder_os_md"] = md
        result["founder_os_path"] = (
            None if dry_run else persist_daily_synthesis(md, today))
    except Exception as exc:  # noqa: BLE001 -- belt: rendering must never break the run
        log.warning("channel_synthesis: daily-synthesis render step failed: %s", exc)
        result.setdefault("founder_os_md", None)
        result.setdefault("founder_os_path", None)
    return result

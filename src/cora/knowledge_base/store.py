...
class KnowledgeBase:
    ...
    def upsert_documents(self, docs: Iterable[Document]) -> int:
        ...
        # (capital-raise, oneamerica, travel-points) OR the LEX copa-bhrf NDA folder
        # (decision §2c, 2026-07-21). Universal chokepoint for every path-bearing
        # source -- static_md (source_id IS the path) and drive_asset (metadata.path
        # is the Drive path). drive_sweep carries no path (source_id = bare file id)
        # and is excluded at enumeration instead (kb_exclusions KB_EXCLUDED_FOLDER_IDS
        # wired into drive_sweep -- covers both classes). Dashboard read layer,
        ...
        for doc in docs_list:
            ...
            if (
                is_dashboard_store_path(doc.source_id)
            ...
            kept.append(doc)
        if dropped:
            log.info("upsert_documents: dropped %d KB-excluded doc(s) (dashboard/NDA store)", dropped)
        docs_list = kept
        if not docs_list:
            return 0

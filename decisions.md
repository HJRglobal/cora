...
## D-074 - Audit Slice 05 (E): finance-tool + egress consistency — one finance-tier gate across all surfaces, close the last raw-HTTP finance egress, bound the dispatch wall-clock (2026-07-06)

Branch `claude/audit-slice-05-finance-egress` off `main`=`61719cf` (D-073 merged). Pushed, NOT merged (Harrison). Suite **5,891 passed** / import smoke OK. Opus 4.8 @ xhigh. Forensic-audit v2 slice 05. Bot-loaded (tool_dispatch / gate changes) -> ONE restart after merge. Every shipped change STRENGTHENS/ALIGNS the D-064 financial firewall + egress boundary; none weaken. **5 of 6 findings shipped; W2-03 DEFERRED (see below) after the mandatory D-051 review caught that its premise is false against live code.**

**W3-04 — one finance-tier gate.** `financial_get_pulse`/`financial_get_close_pack` gated on `_is_finance_channel` (channel endswith `-finance`) while every sibling (cashflow / QBO x5 / osn_pulse) gates on `_is_tier1_channel`. Aligned both to `_is_tier1_channel`; deleted the now-dead `_is_finance_channel`. Verified a WITHIN-firewall alignment, NOT a loosening: every `-finance` channel is still TIER_1 (classify_function -> "finance" -> is_tier_1 True), so nothing LOSES access; the channels that GAIN the pulse .md / close-pack xlsx (leadership/founder/build/any-HJRG) already serve the underlying QBO P&L/BS/AR/AP the file summarizes, so the surface is D-064-consistent. The pulse .md was STRICTER than its own source data — that inconsistency was the bug.

**W2-02 — a DM is TIER_3 at the tool gate.** `_is_tier1_channel` now returns False for a DM (`channel_name=="dm"`) regardless of entity. Before: an HJRG-primary asker short-circuited `channel_classifier.is_tier_1` to True in a DM (HJRG => TIER_1 for any function), so the tool-level TIER_1 gate — documented as the layer-3 backstop that gates live finance DATA "regardless of the deterministic matcher" — FAILED OPEN in DMs, though `user_access.check_access` already pins DMs to TIER_3. The tool layer's only reliable DM signal is `channel_name=="dm"` because `claude_client._dispatch_tools_parallel` does NOT thread the "D..."-prefixed `channel_id` into `dispatch()` (verified); an empty/unknown channel_name is already non-TIER_1. Fixes all finance tools + hjrp_lease_status at the shared chokepoint.

**W3-05 — close the last raw-HTTP finance egress.** `financial_client.upload_report_as_file` PUTs report bytes straight to Slack's upload URL via `httpx.put`, bypassing the `slack_egress` WebClient patch (which wraps only chat_* sends). It now routes content + title through `slack_egress.sanitize_text` (mojibake repair + bare-URL/GID/long-ID redaction) before the PUT. VERIFY-FIRST side-finding: this path is currently DEAD in live Q&A — `channel_id` is never threaded into `dispatch()`, so the cashflow tool's upload branch never fires; the fix closes the boundary regardless of reachability (latent-safe). Reviving Feature 6 (threading channel_id) is a separate egress-expanding change, intentionally NOT done here.

**W3-01 — bound the dispatch wall-clock.** `dispatch()` ran the per-tool `ThreadPoolExecutor` via a `with` context manager whose `__exit__` calls `shutdown(wait=True)` — so on timeout `future.result(timeout=...)` returned but the block then BLOCKED until the hung worker finished, making the timeout tier a no-op. Now: manual executor + `try/except/finally` with `shutdown(wait=False, cancel_futures=True)`. Success path unchanged; a hung tool is abandoned at its timeout (Python can't force-kill a thread, so one worker leaks until it returns — accepted; cleared on the next supervisor restart, far better than blocking every request).

**W2-04 — document the DM Tier-2 guard-trio exemption (no behavior change).** The DM Tier-2 historical-retrieval branch dispatches WITHOUT re-running check_access / sibling / cross-entity. Documented at the dispatch site + grant-consumption point that this is safe by construction — `check_tier2` is fail-closed, the grant is owner-mailbox-scoped (`owned_kb_search`), `_build_grant_context` applies `drop_phi` — cross-ref historical_access.py L31-34; added a named pin test of the three legs. Internal gate preserved.

**W2-03 — DEFERRED (verify-first: the audit finding's premise is false against live code).** The finding: `lex_phi_access.phi_allowed` relaxes PHI for ANY custodian in ANY DM (bare `is_dm`), "safe today only because all non-founder custodians are LEX-primary." The intended fix was to gate the DM relaxation on LEX scope. The mandatory D-051 review (two independent lenses, two independent verifiers, all CONFIRMED HIGH) caught — and I verified live — that `app._handle_dm_qa:1219` resolves the DM entity via `getattr(role, "primary_entity", "")`, but `org_roles.RoleRecord` has NO `primary_entity` attribute (the field is `.entity`; the YAML key is `entity`). So EVERY DM resolves entity="FNDR" (confirmed: `get_role("U0B3PS82G30").entity=="LEX-LLC"` yet the getattr returns "" -> "FNDR"). A custodian's DM therefore NEVER carries LEX scope, so a LEX-scope gate would have refused all four non-founder custodians (Shaun/Jen/Jeff/Aaron) in DMs — refused PHI questions, scrubbed LEX chunks, refused PHI `cora_remember` saves (the D-050 flow). The pre-commit `if is_dm: return True` was COMPENSATING for the entity bug. W2-03 was reverted to the pre-commit (working, documented) behavior. The proper fix is to FIRST repair the `_handle_dm_qa` entity-resolution bug so a custodian's DM loads real LEX scope — a change with broad DM blast radius (context loading / tool exposure / check_access scope for every DM user), out of scope for a finance-egress slice and its own Harrison-gated change (flagged as a follow-up). My own recon script had masked the attribute discrepancy with an `or node.get("entity")` fallback; only the adversarial review surfaced it.

**MANDATORY D-051 review outcome:** 7 lenses (W3-04 loosening / W2-02 DM-tier / W3-01 executor / W3-05 egress / W2-03 PHI / W2-04+tests / cross-preserve), 5 CLEAN, 2 CONCERNS both flagging the same W2-03 HIGH, both CONFIRMED by independent verifiers, 0 refuted. Remediated by reverting W2-03; re-gated green.

**DOCTRINE:**
1. Every tool that gates the SAME data class must share ONE gate. A finance summary artifact (pulse .md, close-pack xlsx) gated more narrowly than the live source data it summarizes is a latent inconsistency, not extra safety — align the summary to the source's TIER_1 boundary; do not invent a per-tool `-finance`-suffix sub-gate.
2. A per-surface security tier must be pinned by the SURFACE, not derived from the asker's identity/entity. Deriving a DM's tier from the asker's org-roles entity let an HJRG-primary user flip a DM to TIER_1; the roster-independent rule "a DM is TIER_3" belongs at the tool gate, matching what check_access already pins. Know your DM signal: `channel_id` is NOT threaded into `dispatch()`, so `channel_name=="dm"` is the tool-layer DM marker.
3. A single sanitizing egress boundary only holds if EVERY send path funnels through it. A raw `httpx.put` file-upload bypasses a WebClient-method patch by construction; sanitize at the upload SITE. Close the boundary even on a currently-unreachable path — reachability changes silently, the leak would not.
4. A timeout is only real if the wall-clock is bounded. A `with ThreadPoolExecutor` defeats its own `future.result(timeout=...)` because `__exit__`'s `shutdown(wait=True)` blocks on the hung worker; use `shutdown(wait=False, cancel_futures=True)` and accept a leaked worker over a blocked request.
5. VERIFY-FIRST beats an audit finding whose premise is false against LIVE code — even when the finding is "obviously safe." W2-03 assumed a custodian's DM carries their LEX primary; live code always passes FNDR (a wrong-attribute-name bug), so the "tightening" would have shipped a HIGH PHI-access regression that a green suite hid. Confirm each finding's premise against live code AND live data (a get_role attribute, not just the YAML key) before changing behavior; a benign-looking `getattr(x, "wrong_name", default)` fails silently to the default.
6. When a correct fix is blocked by an out-of-scope root-cause bug, DEFER the fix rather than ship a regression or expand scope. The W2-03 gate is correct only after `_handle_dm_qa` loads real DM scope; fixing that has broad DM blast radius and belongs in its own reviewed change. Revert to the documented working behavior and flag the root cause.
7. A test that constructs a stand-in for a production object (SimpleNamespace with `primary_entity`, or a hand-passed entity a real code path never emits) can hide an attribute-name / data-shape bug and go green. Integration-pin the guard with the REAL object the live path produces (a real `org_roles.RoleRecord`), or the suite validates a fiction.

## D-075 - DM entity-resolution fix: resolve a DM's entity from RoleRecord.entity + re-enable the W2-03 LEX-scope PHI gate (2026-07-06)

Branch `claude/dm-entity-scope-fix`, **STACKED on `claude/audit-slice-05-finance-egress`** (commits `04e04ab` fix + `76660df` D-051 remediation). Pushed, NOT merged (Harrison). Suite **5,898 passed** / import smoke OK. Opus 4.8 @ xhigh. Fixes the root cause that DEFERRED audit W2-03 (D-074). Bot-loaded (app.py + lex_phi_access) -> ONE restart after merge. STRENGTHENS/ALIGNS the LEX PHI wall + entity firewall; never weakens.

**THE BUG.** `app._handle_dm_qa` resolved the DM entity with `getattr(role, "primary_entity", "")`, but `org_roles.RoleRecord` has NO `primary_entity` attribute (the field is `.entity`; the YAML key is `entity`). So EVERY plain DM silently resolved entity="FNDR" (the unknown-user fallback), contradicting the handler's own docstring. Verified live: `get_role("U0B3PS82G30").entity=="LEX-LLC"` yet the getattr returned `""` -> `"FNDR"`. Discovered by the slice-05 D-051 review (D-074), which is why W2-03 was deferred.

**BLAST-RADIUS TRACE (verify-first, real roster).** Under the bug, every teammate whose `allowed_entities` excludes FNDR (13 users: Alex/Tommy/Matt/Micah/Shaun/Jen/Jeff/Aaron/Jake/Larry/Daniel/Demi/Elena/Sara) was REFUSED in DMs (check_access blocked entity="FNDR"); the fix ENABLES scoped DM Q&A for them at their real entity, with TIGHTER context/tool/cross-entity scoping than the FNDR catch-all. Users whose real entity is already FNDR/HJRG (Harrison/Justin/Hannah/Alina/Jerry/Eric-via-carveout) are unaffected. Only ONE net behavior change: Jason Dorfman (the sole external user, F3E, unlisted in permissions) loses FNDR catch-all DM access and is refused — the SECURE direction, reversible via a one-line permissions.yaml add if scoped F3E DM access is wanted.

**THE FIX.**
- `app.py _handle_dm_qa`: read `getattr(role, "entity", "")` (the real field). Unknown/unmapped -> "" -> FNDR (catch-all, unchanged); Harrison still hard-pinned FNDR.
- `lex_phi_access.phi_allowed`: re-apply the W2-03 LEX-scope DM gate + single-founder carve-out (reverted in slice-05/D-074 as premature). Now CORRECT: a custodian's DM finally carries LEX scope (entity=LEX-LLC), so the four LEX custodians relax in DMs and the guarantee is roster-independent.
- `user_access.has_unrestricted_entity_access` + carve-out: a portfolio-wide user (`allowed_entities: all`) resolves their DM to the HJRG aggregator, NOT their narrow home entity, so cross_entity_guard doesn't redirect cross-entity questions they're authorized to ask (D-051 LOW finding — hit Eric/cross-entity-finance + Alina/HR). Gated strictly on allowed=all (only Harrison/Justin/Eric/Alina), so it never broadens a narrow user; Harrison stays FNDR-pinned.

**COUPLING WITH SLICE-05 (why stacked).** Fixing the entity resolution ACTIVATES the latent W2-02 finance-in-DM exposure: an HJRG-primary user's DM now resolves to HJRG, and `channel_classifier.is_tier_1` short-circuits True for HJRG, so the finance-tool tier gate would FAIL OPEN in their DM. Slice-05's W2-02 pin (`_is_tier1_channel` returns False for `channel_name=="dm"`) closes exactly that and is inherited on this stacked branch. **Merge slice-05 first (or merge this branch, which contains it).** Merging this WITHOUT slice-05's W2-02 would open finance-in-DM for HJRG-primary users.

**MANDATORY D-051 review.** 6 lenses (over-grant / refusal-regression / w2-03-phi / w2-02-coupling / tests-and-semantics / cross-preserve): 5 CLEAN, 1 LOW (the allowed=all cross-entity-redirect, remediated by the HJRG carve-out), 0 CONFIRMED MEDIUM+, 0 refuted. The over-grant, PHI, coupling, and test-validity lenses were all CLEAN.

**FALSE-GREEN TESTS FIXED.** `test_dm_qa_routing.py` and `test_surface_guard_parity.py` mocked `get_role` with `SimpleNamespace(primary_entity=...)` — a shape production `RoleRecord` never has — which HID this bug (the surface-parity DM tests went RED under the fix, proving the mock was lying). Both now use a real `org_roles.RoleRecord`. Added an end-to-end integration test driving `_handle_dm_qa` with a real `RoleRecord` + the REAL `lex_phi_access` gate: a LEX custodian's DM resolves LEX-LLC and flows `phi_custodian=True` into `check_access`; a non-custodian does not.

**DOCTRINE:**
1. A `getattr(obj, "field", default)` on the WRONG attribute name fails silently to the default — it never raises, so a field rename / typo produces plausible-but-wrong behavior a green suite won't catch. Read the real field; when a value is load-bearing (an access scope), integration-pin it against the REAL object the live path emits, never a hand-built stand-in.
2. A test double must match the production object's SHAPE. A `SimpleNamespace(primary_entity=...)` standing in for a `RoleRecord` (field `.entity`) turned the entire DM surface green while every real DM silently mis-resolved to FNDR. Prefer the real dataclass in guard/scope tests; a mock that can't fail the way production fails is a false witness.
3. When a bug's fix ACTIVATES a latent exposure elsewhere (here: correct DM entity resolution un-masks the W2-02 HJRG finance-in-DM hole that the always-FNDR bug had been hiding), the fix and the exposure's closure are COUPLED — ship them together (stack the branches) or the "fix" opens a hole. Trace what a correctness fix un-masks, not just what it corrects.
4. Resolve a per-surface scope to match the user's AUTHORIZATION, not just their home label. A portfolio-wide (allowed=all) user narrowed to a single home entity gets cross-entity questions redirected they're entitled to ask; resolve them to an aggregator scope. Gate such a broadening strictly on the authorization signal (allowed=all) so it never widens a narrow-scope user.
5. A long-latent "it works" can be a masked refusal. The always-FNDR bug meant every non-FNDR-authorized teammate was silently REFUSED in DMs for weeks; nobody noticed because they use channels. A trace against the REAL roster (not the happy path) surfaced it — audit the population, not the demo user.

## D-076 - Audit Slice F: message-handler-path hygiene (W1-01 double-dispatch, W1-02 dead reaction gate, W3-02 image-gen timeouts) + folded W-DMQ gap-ask misroute (2026-07-06)

Branch `claude/audit-slice-f` off `main`=`4163a7f` (D-075). Pushed, NOT merged (Harrison). Suite base **5,898** -> **5,917 passed** / import smoke OK. Opus 4.8 @ xhigh. All changes bot-loaded (app.py / tools/tool_dispatch.py / gap_autofill.py) -> **ONE restart after merge**. Preserve-list intact (entity firewall, D-064 finance+TIER_3, D-068 thread parity, PHI/LEX wall, D-050, D-075 DM entity, egress/source-opacity, D-011, D-034); nothing weakened. 3 commits: `61293a4` (W3-02) + `39b9bee` (W1-01/W1-02/W-DMQ) + `d0190a0` (D-051 review remediation + residuals doc).

**W1-01 (MED) — @mention-in-active-thread double-dispatch.** An @mention posted as a reply INSIDE an already-active thread is delivered by Slack as BOTH an app_mention (-> handle_mention, a full answer) AND a message event; Path 2's `active_thread_store.is_active` was TRUE, so it dispatched a SECOND full answer (doubled LLM call + duplicate reply, on mention-polluted text since Path 2 never strips the leading `<@Uxxx>`). FIX: before Path 2, skip when the reply text contains `<@{Cora bot id}>` (handle_mention owns those). Scoped to Cora's OWN id — a follow-up mentioning a teammate still routes; unresolved bot id fails OPEN (never drops a real follow-up). Placed AFTER Path 0 (note-confirm) and Path 1 (correction), which handle_mention does NOT own, so a "@Cora yes"/correction reply still reaches them.

**W1-02 (LOW) — dead redundant reaction gate.** `_handle_reaction` re-fetched item_user/bot_user_id and repeated the identical `item_user != bot_user_id: return` after the top gate already enforced it (values cannot change in between) — dead code — and carried a stale "Runs before the bot_user_id gate" comment that was false (the HubSpot-reaction block runs AFTER that gate, correct because the pending-reaction DM is Cora-authored and get_pending_reaction is keyed on Cora's own DM ts). FIX: removed the re-check; corrected the comment. +3 behavioral tests locking the single-gate invariant (no reaction test existed before).

**W3-02 (HIGH) — image-gen dispatch timeouts below internal budgets.** f3_generate_image/f3_batch_image_run had no `_TOOL_TIMEOUTS` entry (15s default) and f3_create_image was 25s — all shorter than photoroom_client's ADDITIVE internal httpx budgets (main download 30s + optional ref download 30s + generation POST 60s). Since W3-01 (D-074) made the dispatch timeout a TRUE wall-clock bound, a real generation was abandoned mid-flight and returned a spurious "Tool timed out" while the worker kept spending. FIX: f3_generate_image=90, f3_create_image 25->100 (adds a Haiku brief->spec via spec_generator), f3_batch_image_run=180 (N specs in series; a large batch exceeds this and completes in the background — images still land in Drive; true fix for large batches is async fire-and-report, tracked). Each dispatch() builds its OWN single-worker executor, so raising a timeout cannot starve a shared pool. Regression test pins timeout > 60s PhotoRoom budget + the create>=generate, batch>=create ordering. VERIFY-FIRST: PHOTOROOM_API_KEY is present (production endpoint) and there are historical generation log lines — the tools are live-wired, low-volume.

**W1-03 (LOW) — NO code change.** W2-04 (Slice E / D-074) already documented the Tier-2 DM historical-retrieval guard-trio exemption in app.py and pinned it (`test_w2_04_guard_trio_exemption_contract`, `test_app_has_dm_retrieval_branch`). Confirmed, not duplicated.

**W-DMQ (folded) — DM gap-ask misroute; root cause NOT what the hand-off hypothesized.** Verified live 2026-07-06: a genuine finance question DM'd to Cora note-captured ("routed your answer to Harrison") once, then identical re-sends went to Q&A. The hand-off guessed a stochastic LLM classifier; the REAL cause is DETERMINISTIC — `gap_autofill.match_pending_ask` greedily captures the next non-shift TOP-LEVEL DM as the answer to a lone outstanding knowledge-gap ask, regardless of content, and `record_ask_answer` then consumes the ask (state->ANSWERED). So the first DM consumed the ask; later identical DMs found no live ask and correctly reached Q&A — explaining the apparent non-determinism. It is NOT the cora_remember/note path (that takes 38-53s, not ~2s, and its ack differs). Proof: the observed ack string is `gap_autofill.py:670`. FIX: new `gap_autofill.looks_like_question()`, folded into `allow_toplevel` at the app.py im-branch caller, so a clearly-interrogative top-level DM routes to Q&A instead of being captured. A reply typed in the ask's OWN thread still ALWAYS matches (explicit intent wins). Declines ("no idea") stay declarative and still capture. Untouched: D-050 cora_remember PHI matrix, D-075 DM entity resolution, D-064 / W2-02 finance-in-DM gate, OSN shift routing.

**MANDATORY D-051 review (6 lenses -> 2 skeptics/finding) + a focused 2-skeptic re-verify of the remediation.** W1-01 / W1-02 / W3-02 / preserve-list / completeness lenses: CLEAN (0 findings). W-DMQ lens: caught a REAL MED regression the first cut introduced — `looks_like_question` flagged auxiliary-LED declarative ANSWERS ("Has to go through Hannah", "Should be the Tucson location", "Will check with Justin") and proper-noun collisions ("Will Rogers handles that", "May Chen owns it") as questions, LOSING a teammate's real gap answer (WORSE than pre-change). REMEDIATION (`d0190a0`): split the heuristic — leading wh-words unconditional; an auxiliary opens a question ONLY when a question SUBJECT follows (do WE / is THE / can YOU). Re-verify confirmed the proper-noun collision cases are now safe and surfaced two ACCEPTED residuals (below). +3 regression tests.

**ACCEPTED RESIDUALS (documented, both recoverable — a lexical classifier over free prose cannot perfectly separate a question from an answer; the feature is LOW/SAFE and Harrison-gated).**
- False-positive (dangerous direction, occasional): an ANSWER ending in a confirmation tag ("... , right?") or leading with "aux + the/that" reads as a question and is routed to Q&A instead of captured — the ask stays PENDING (re-escalated / digest picks it up; a threaded reply always captures). This is a narrow regression vs the pre-change greedy capture, accepted because it is recoverable and the alternative (dropping the trailing-'?' rule) undercuts the bug fix's own purpose.
- False-negative (safe direction): an aux-led question about a NAMED person ("did Justin send it") or an imperative ("pull up the Q1 P&L") without a trailing '?' is captured as a gap answer -> a Harrison-gated bogus proposal he -1's at the 7am review (annoyance, never a write).
- ROBUST FOLLOW-UP (product/UX, not this cleanup): drop top-level auto-capture entirely and require a THREADED reply to the ask — eliminates all question/answer ambiguity. Left for Harrison to decide.

**DOCTRINE:**
1. A tool's DISPATCH timeout must exceed the SUM of the tool's own internal network budgets, or the dispatch layer pre-empts the tool's own error handling — the user gets a generic "timed out" while the (post-W3-01, abandoned) worker keeps spending. Set it above the internal ceiling; pin it with a test that references the internal budget so a future edit can't silently drop it back under.
2. An interrogative HEURISTIC over free text is caught between over- and under-matching (same class as the D-051 PHI-redactor doctrine). Leading auxiliaries front BOTH questions ("do we have X") and declarative answers ("Has to go through Hannah") — a bare auxiliary-lead over-matches and silently drops real answers. Require a question SUBJECT after the auxiliary; keep wh-words unconditional; treat trailing '?' as the highest-precision signal. When residual error is unavoidable, document BOTH directions and bias toward the recoverable one; do not chase perfect separation with ever-more regex arms.
3. A hand-off's stated root-cause hypothesis is a lead, not a fact. W-DMQ was framed as a stochastic LLM classifier; the ack string (`gap_autofill.py:670`) + the ~2s latency proved a deterministic state-based matcher. Trace which function emitted the observed message before designing the fix — the deterministic cause admits a small deterministic fix.
4. A green suite does not license shipping. The mandatory D-051 fleet caught a MED regression (dropped gap answers) that all 5,914 tests passed over, because the first tests only covered noun/pronoun-leading answers. Adversarial diff review + a focused adversarial re-verify of the fix itself remain the net that catches what tests don't.

**RESTART/MERGE MATRIX:** fast-forward `main` after review + push; ONE restart after merge (all bot-loaded). Live smoke: (1) @mention as a reply in an active thread -> exactly ONE reply; (2) an image generation completes (no spurious timeout); (3) a clearly-interrogative DM reaches Q&A; (4) a genuine gap-ask ANSWER DM still captures. Full record: `_shared/projects/cora/2026-07-06_fndr_cora-slice-f-cascade-report.md`.

---

## D-078 - MEGA smoke-test remediation: channel-scope content guard (F-08 family) + calendar/asana staged-write server-side binding + phantom-destructive guard + tool-selection/freshness/formatting fixes (2026-07-12)

Branch `claude/mega-smoke-remediation` off `main`=`c9dbe59`. Pushed, NOT merged (Harrison). Suite base **6,438** -> **6,529 passed** / import smoke OK. Opus 4.8 @ xhigh. 8 commits (`e944c8f` fmt / `b898166` guard / `34fbfdf` routing / `8871824` calendar / `6459482` write-net / `050cf10` freshness / `fd274e0` briefing / `f32d91c` D-051 remediation). Source of truth: `_shared/projects/cora/2026-07-12_fndr_cora-mega-smoke-FINDINGS.md` (F-01..F-23 + FR-1). Mostly BOT-LOADED -> **ONE restart after merge**; F-09 + F-14a are script-side (live at next fire). Preserve-list intact (entity firewall, D-064 finance+TIER_3, D-068 thread parity, PHI/LEX wall, D-050, D-075 DM entity, egress/source-opacity, D-011, D-034); nothing weakened. NOTE: D-078 is provisional -- the concurrent `claude/dashboard-read-layer` branch also claims the next number; Cowork assigns the final numbers at promotion.

**Headline (F-08 family, ANCHOR): the KB/personal-context ANSWER path had NO channel-scope content guard.** Cora enforced confidentiality at the TOOL layer + pre-LLM keyword guards, but because authorization is per-PERSON (Harrison is authorized for everything), the model surfaced confidential content into whatever CHANNEL he asked in -- OneAmerica figures + $25M capital terms into #cora-build (F-08), company revenue into TIER_3 #f3-athletes (F-12), F3E creator CRM into #osn-leadership AFTER a correct tool self-refusal (F-13), travel points (F-10), dashboard enumeration (F-11). NEW `src/cora/channel_content_guard.py` = the OUTBOUND twin of the retrieval PHI scrub: `guard_outbound` evaluates the COMPOSED answer against the channel, keyed on CHANNEL not asker (fires even for the founder), and refuses a confidential class the channel does not permit. Classes: personal_insurance / capital_program / travel_points (DM-to-Harrison, channel scope DELEGATED to `dashboard_access` = single source of truth) - creator_crm / content_pipeline (split; different dashboard scopes) - company_financials (TIER_1 / named founder-ops channel / DM). Wired at EVERY `_dispatch_qa` serve site (cache-hit + non-streaming + streaming-final + each streaming FRAME); the ORIGINAL is cached, guarding happens at serve (the semantic cache is entity-keyed, NOT tier-keyed, so a TIER_1-built answer can be cache-served into a TIER_3 channel of the same entity); skipped on the Tier-2 owner-mail grant path (1:1 DM, already access-controlled).

**F-23 fabricated destructive success (write-net).** Cora rendered a full staged-DELETE preview + "Task deleted permanently" with ZERO asana_delete_task tool_use -- the task survived. Asana complete/delete now use a server-side pending store (identity = the gid resolved+stashed on preview, popped+executed on confirm -- never a confirm-turn LLM echo); the claude_client narration net generalized from the single Shopify tool to a SET; + a phantom-destructive-claim guard: when NO contract-write sentinel fired but the model text ANNOUNCES a destructive Asana action, override with a truthful correction. Calendar create (F-05, was honor-system -> booked on first call) + a NEW confirm-gated calendar_delete_event (F-06) use the same server-side-binding pattern.

**Other slices.** F-02 calendar-read Sonnet routing + `## Calendar reads` directive in all 17 prompts (the model fabricated a calendar outage on Haiku). F-01/F-16/F-22/parked-1 composite/dashboard tool families -> Sonnet + mandatory-tool prompt directives. F-04 hooked `normalize_slack_bold` into the universal `slack_egress.sanitize_text` (literal ** on verbatim-tool + streaming frames -- the only real leaks). F-03 asana_get_my_tasks bounded to 10. S3.4 sheet-name double-article. F-17 office inventory reads -> live Shopify (read/write parity, Excel fallback). F-21 recency-first sort for "latest email". F-09 static-md content-sha256 trigger (a redaction shrinks content without advancing mtime). F-14a bounded the untimed briefing `messages.create` (0xC000013A = external SIGKILL, not a Python crash). F-19 reconciled the stale repo Slack manifest (message.channels/groups). F-18 confirm wording.

**VERIFY-FIRST corrections (audit vs live code):** F-14b WRONG (finance-digest already repointed to #founder-finance in the YAML -- no code); F-15 WRONG (briefing has no cashflow-latest.json dependency); F-04 "4 formatters emit **" / "haiku fast-path bypass" claims WRONG (the real leaks are verbatim-tool prose + streaming frames); F-19 the "zero processing" claim is likely a stale-manifest artifact (the live app almost certainly has the subscriptions -- Harrison verifies at api.slack.com).

**MANDATORY D-051 review (6 lenses, find/verify/synthesize, 19 agents) found 8 confirmed defects, ALL in touched files -- fixed + re-gated (`f32d91c`):**
- **#1 HIGH (a leak this branch INTRODUCED):** company_financials failed OPEN in every UNMAPPED channel -- `entity_router.route()` returns "FNDR" for the catch-all, and the initial `entity in {FNDR,HJRG}` founder exemption then permitted it, so a revenue figure would leak into any unmapped multi-member channel (e.g. #investor-updates -- the exact F-12 class). FIX: an explicit founder-ops channel-NAME allowlist (`_FOUNDER_FIN_CHANNELS = {founder-operations}`); TIER_1 founder channels still pass on tier; #founder-operations (S2.10) stays allowed. Mirrors `dashboard_access.is_mapped`'s catch-all guard.
- **#2-#6 MED:** capital_program missed "capital raise" + hard-required "seat" (leak); is_recency_query over-matched "email from last week" -> date-sort evicted the relevant email (wrong answer); the phantom-destructive guard clobbered FACTUAL third-person "that task was deleted on 6/3" with an affirmatively FALSE "I didn't change anything" -> first-person-scoped it; travel_points bare "A-List" + capital_program bare "%" over-refused legit F3E vocabulary.
- **#7-#8 LOW:** calendar-read Sonnet coverage gap ("show me my calendar"/"when's my next meeting"); `on \w+day` matched holiday/birthday.

**DOCTRINE:**
1. A channel-scope content guard is the OUTBOUND twin of the retrieval PHI scrub: authorization is per-PERSON but exposure is per-CHANNEL, so guard the COMPOSED answer keyed on the channel (tier/entity), not the asker -- a tool refusal + a "DM-only" footer are cosmetic when the model can answer the same thing from context in the same turn.
2. NEVER key a founder/entity exemption on `entity == FNDR`: `route()`'s trailing catch-all returns FNDR for EVERY unmapped channel, so the exemption fails OPEN. Use a channel-NAME allowlist (or gate on `is_mapped()`), mirroring dashboard_access.
3. A phantom-success guard must be FIRST-PERSON-scoped (Cora claiming SHE just acted), or it clobbers factual third-person status reports with an affirmatively false statement -- worse than an over-refusal.
4. Staged-write identity binds SERVER-SIDE (a pending store keyed (user,channel), resolved on preview, popped on confirm), never an LLM echo of the confirm-turn fields -- now uniform across Shopify + calendar + Asana. A first-call confirmed=true re-previews; a restart-cleared store re-previews; never a blind write/delete.
5. The semantic cache is entity-keyed, not tier-keyed: cache the ORIGINAL answer and re-apply the channel guard at EVERY serve site (cache-hit included), or a TIER_1-built answer replays into a TIER_3 channel of the same entity.

**RESTART/MERGE MATRIX:** fast-forward `main` after review + push; ONE restart after merge (bot-loaded: channel_content_guard/app.py/tool_dispatch/claude_client/model_router/reply_formatter/slack_egress/historical_access/store/context_loader + 17 prompts). F-09 (incremental_sync_static) + F-14a (run_daily_briefing) are script-side. Harrison live-verify F-19 bot_events at api.slack.com. FR-1 (attendee-recap PUSH) STAGED, not built (a feature; reuses meeting_actions attendee gate + LEX rails + staged-create, question-only, preserves D-054). Full record: `_shared/projects/cora/2026-07-12_fndr_cora-mega-smoke-remediation-CASCADE-REPORT.md`.

---

## D-080 - AI-Visibility 4th brand: Harrison Rogers (hjr) at measurement parity (2026-07-14)

Branch `claude/ai-visibility-hjr-brand` off `main`=`8509013`, MERGED to `main` @ `e85ea34`. 3 commits (`cbb70a0` basket+tests / `c19662b` card+tool / `d56e813` D-051 remediation). Suite base **6,641** -> **6,647 passed** / import smoke OK. Opus 4.8 @ xhigh + ultracode. NUMBERING: shipped as provisional D-079; renumbered to **D-080** because D-079 was taken by the F-23 model-independent-confirm arc (promoted 2026-07-12). Source of truth + full record: `_shared/projects/cora/2026-07-14_fndr_cora-ai-visibility-hjr-brand-CASCADE-REPORT.md`.

Adds Harrison Rogers (brand key `hjr`, display "Harrison Rogers") as the 4th brand in the in-house AI-visibility engine (D-077 lineage) at FULL MEASUREMENT parity with the frozen F3 brands (energy/pure/mood). Harrison approved his personal brand riding the F3E lane/channel/card. Scope was measurement parity only (no Part-B content loop).

**What shipped:**
- `data/maps/ai-visibility-prompts.yaml`: new `hjr` brand -- 12 HJR-* prompts (discovery 7 / comparison 1 / branded 4), aliases (Harrison Rogers / HarrisonJRogers / Harrison J. Rogers / HJR Podcast / THE HJR PODCAST), fail-closed PERSON disambiguation (namesake excluded, same posture as the F3-Nation rule), competitor set mirroring Otterly (Alex Hormozi / Codie Sanchez / Patrick Bet-David / Steven Bartlett / Andy Frisella / My First Million). The shared "Who makes F3 Energy and who founded it?" question is a NEW HJR-D01 id (unaided for hjr); ENG-B06 untouched. F3 instrument stays FROZEN at 89 prompts; total now 101.
- `report.py`: `hjr` added to `_BRAND_ORDER`/`_BRAND_LABEL`; new `_BRAND_SHORT` makes the gap line brand-aware (F3 brands keep "but F3 isn't" byte-identical; hjr reads "but Harrison Rogers isn't"). Card title "F3 AI Visibility" retained by Harrison's choice.
- `tool_dispatch.py`: `f3e_ai_visibility` DESCRIPTION names the founder brand + a trigger; entity/channel GATE UNCHANGED (F3E + FNDR/HJRG only, no guard loosened).
- `classifier.py`: judge scaffolding made target-agnostic ("a brand OR a person"; is_correct_brand references the Disambiguation generically) -- F3 behavior unchanged (per-brand disambiguation still carries the beverage constraint).

**VERIFY-FIRST correction (kickoff premise #4 was wrong):** otterly_client has NO report-id map. `fetch_aio_slice` resolves the AIO report dynamically by ALIAS via `_match_report_for_brand`, so the "Harrison Rogers" report auto-matches from the yaml aliases -- zero otterly_client edits, already fail-soft.

**Person-brand AIO gate (the notable decision):** the D-051 review found the Otterly Google-AI-Overviews leg counts brand-name mentions with NO wrong-person disambiguation, unlike the 4 direct engines (each Haiku-judged with fail-closed is_correct_brand). For a PERSON brand a namesake Harrison Rogers could silently inflate the merged headline composite. FIX: `_PERSON_BRANDS = {"hjr"}` in the runner gates the AIO 5th-engine merge for person brands; hjr is scored on the 4 disambiguated direct engines only. The composite FORMULA is identical to F3 (presence 40 / SoV 25 / position 20 / sentiment 15); only the un-disambiguatable AIO merge is excluded. REVERSIBLE (drop the key once the Otterly report's brand-variation config is confirmed to exclude namesakes). This is a DELIBERATE, DOCUMENTED deviation from the kickoff's "identical AIO merge" point -- flagged for Harrison to override if he prefers the merge despite the namesake risk.

**UPDATE 2026-07-14 (same day) -- AIO gate DISABLED for hjr (Harrison's decision; branch `claude/ai-visibility-hjr-aio-enable`, runner-side, NO restart):** Harrison ACCEPTED the namesake-inflation risk and chose full 5-engine parity with the F3 brands. `_PERSON_BRANDS` is now EMPTY (`frozenset()`), so hjr's Otterly AIO slice IS fetched + merged into its headline composite like every F3 brand. The gate MECHANISM is kept in place, documented, for any future person brand (add its key back to re-gate). Fail-soft preserved: the Otterly "Harrison Rogers" report was created 7/13 and may be thin -- a missing/sparse slice simply is not merged (never zeroes or crashes the composite). Revisit only if a brand's AIO presence looks implausible vs its judged direct-engine presence. The pin test flipped from "excludes AIO" to "includes AIO" + a thin-slice fail-soft pin.

**Live acceptance (1-run perplexity_sonar, scratch DB, no Slack post):** hjr composite 54.43 (= direct-only, AIO gated), presence 41.67% (5/12 hits), SoV 62.5%, sentiment 92.0, unaided 12.5%; branded intent 100% (all 4 branded prompts hit position 1 -- disambiguation surfaces the correct Harrison Rogers + HJR Podcast), discovery 14% (founder-of-F3 prompt surfaced him unaided). Prod DB untouched (scratch), so the live tool/card keep the 7/13 3-brand scan until the next Monday scan includes hjr.

**D-051 review (6 lenses, find/verify, 8 agents):** frozen-instrument / scoring-store / card-tool-guard / deploy-doctrine CLEAN; otterly-aio + test-adequacy found 4 (2 MED: AIO namesake gate + beverage-locked judge; 2 LOW: report dict-coverage test + stale runner text) -- all fixed + re-gated in `d56e813`.

**DOCTRINE:**
1. Making a MEASUREMENT instrument's target a PERSON (not a product) changes the disambiguation surface: a namesake can pollute any name-matched signal. Aggregate third-party signals we cannot re-judge per-answer (the Otterly AIO leg) CAN be gated out of a person's scored headline via a reversible, data-driven lever (`_PERSON_BRANDS`); whether to gate is an OPERATOR decision (Harrison accepted the risk for hjr and chose full parity -- see the 2026-07-14 UPDATE). Keep the lever + a fail-soft path so a thin third-party report never zeroes or crashes the composite.
2. A shared LLM-judge scaffolding hardcoded to one target TYPE ("beverage brand") silently breaks a new target type (a person) -- it contradicts the per-brand disambiguation in the rendered prompt and can drop valid hits. Keep judge scaffolding target-agnostic; let the per-brand disambiguation carry the specifics.
3. A brand-keyed display/order table (`_BRAND_ORDER`/`_BRAND_LABEL`) that direct-indexes is a KeyError/silent-omission trap when the data (basket) grows; pin it to the data source with a coverage test.

**RESTART/MERGE MATRIX:** fast-forward `main` after review + push; ONE restart after merge for the INTERACTIVE tool only (bot-loaded: report.py + tool_dispatch description + the in-process basket cache). The weekly Slack CARD is runner-side (fresh scan process) and needs NO restart -- it renders 4 blocks at the next Monday 10:15 AZ scan regardless. yaml/classifier/runner changes are runner-side (live at next fire). No task re-registration needed. Preserve-list intact (F3 89-prompt instrument frozen, entity/channel gate unchanged, D-011 no autonomous canonical writes, source-opacity, PHI-OFF marketing posture).

## D-081 - Missed-message catch-up must reconstruct the live pipeline READ-ONLY (2026-07-15)

Backfilled 2026-07-17 (repo log had a gap; founder canon already carries D-081). Branch `claude/missed-message-catchup` @ `4f1e87c`, PUSHED, awaiting merge + ONE bot restart. Suite 6703. Source of truth: `_shared/projects/cora/2026-07-15_fndr_cora-missed-message-catchup-build-spec.md` + memory `project_cora_missed_message_catchup`.

After an outage (Socket Mode drops events while the bot is disconnected), Cora silently loses every @mention/DM/thread ask over the down window. `src/cora/missed_message_catchup.py` reconstructs those asks, replays the handler pre-LLM guard sequence (user_access -> help -> sibling_guard -> cross_entity_guard), WRAPS `app._dispatch_qa` to draft answers, and DMs Harrison per-message Send/Edit/Skip cards -- nothing posts without his tap (D-011). Runner `scripts/run_missed_message_catchup.py` defaults DRY-RUN; `--send-cards` DMs the cards. Dry-run acceptance on the real 7/14->7/15 incident found 2 missed @mentions and proved channel-content-guard inheritance; LTS/LBHS PHI + personal channels excluded.

**DOCTRINE (D-081):** a reconstruction/replay of the live pipeline MUST be read-only. The D-051 review caught a HIGH I introduced -- draft capture ran the live TOOL-enabled pipeline, so a reconstructed "yes" after a pre-outage proposal executed REAL calendar invites / gmail drafts / tracker writes even in dry-run. Fixed via `CORA_EVAL_MODE=1`: tools_for_entity->[], dispatch refuses, the F-23 confirm interceptor short-circuits, and active_thread_store.register / _try_cache_store / gap+feedback loggers no-op. Every real-write path (tools AND the confirm interceptor) must honor the eval flag; drafts are KB/context-only.

## D-082 - PM-hub Phase 1: conversational task edits (staged-write, ownership-scoped) + adoption instrumentation (2026-07-15)

Backfilled 2026-07-17 (repo log had a gap; founder canon already carries D-082). Branch `claude/pm-hub-phase1` off `4f1e87c`, PUSHED, awaiting merge + ONE bot restart. Suite 6810. Source of truth: `_shared/projects/cora/2026-07-15_fndr_asana-vs-cora-pm-hub-decision-brief.md` + memory `project_cora_pm_hub_phase1`.

Make Cora the measurably primary task interface in Slack (Asana stays the backing store). Slice 1: three staged-write, ownership-scoped tools (`asana_update_task` / `asana_add_comment` / `asana_add_subtask`) reusing the F-23 server-side pending store (confirm executes the STASHED payload, never an LLM echo); ownership reuses the `_resolve_asker_task` WS5 gate (act only on a task you own; founder + FNDR/HJRG cross-entity-exempt by design); LEX content PHI-scrubbed fail-closed. Slice 2 (the load-bearing deliverable): `src/cora/pm_metrics.py` -> `logs/pm-actions.jsonl` + a weekly adoption digest (Mon 08:20 AZ DM to Harrison) = the Phase-2 go/no-go. No new autonomy (every edit is preview->confirm), no Asana governance changes.

**DOCTRINE (D-082):** a staged-write log/metric sink that gates PHI on the CHANNEL entity is WRONG when the tool acts cross-entity (a LEX task title can't be gated by a non-LEX channel) -- gate on the TRUE task entity or omit content entirely. pm-actions.jsonl persists gid + entity + action ONLY, never a task title. Slice 3 (per-person dashboard) + Phase 2 (Cora as system of record) deferred.

## D-083 - G: Drive-mount resilience: a local-mount read is a HANG risk, bound it on a disposable thread (2026-07-16)

Backfilled 2026-07-17. Branch `claude/drive-mount-resilience`, MERGED to `main` @ `502906d`. Suite 6831->6862. Source of truth: memory `project_cora_drive_mount_resilience`.

Twice in two days (7/14, 7/15) the Google-Drive `G:` mount blipped (unmount->remount ~30s) and the always-on bot FROZE -- last heartbeat then nothing, no traceback, ~9.5h dark. Root cause: a blocking read against the vanished mount froze the WHOLE interpreter (heartbeat file + logs are on C:, so the heartbeat thread never touches G: -- the only way it stops is an interpreter-wide freeze). New module `src/cora/drive_io.py`: every LOCAL Founder-OS read/write runs on a disposable daemon worker joined with a per-attempt deadline, retried over a bounded window, fail-typed `DriveUnavailable` (OSError subclass), with a process-wide circuit breaker. Bot request path (context_loader incl. the per-request known-answers mtime stat) degrades to cached/degraded and KB retrieval (cora_kb.db on C:) keeps answering; scheduled jobs (strategy_memo/materializer/nudge_ledger/session_capture) fail soft; a reachability breadcrumb daemon logs G: transitions. The `cora-watchdog` (heartbeat auto-restart, shipped 7/16) is the recovery half.

**DOCTRINE (D-083):** a local Drive-mount read is a HANG risk, not just an error -- bound it on a disposable thread + fail-type it; the heartbeat/loop must be provably independent of any G: touch. The Drive **API** connectors (drive_sweep/drive_connector/attachment_filer) are network calls, NOT the local mount -- exempt. Dual import-tree gotcha: production + tests both `from cora import ...` so DriveUnavailable class identity matches.

## D-084 - FNDR/HJRG context budget: slim the wholesale founder brief + a pre-send token-budget guard (2026-07-17)

Branch `claude/fndr-context-budget` off `main`=`502906d`, PUSHED, awaiting merge + ONE bot restart (claude_client + context_loader are bot-loaded). Suite 6884. Source of truth: `_shared/projects/cora/2026-07-17_fndr_cora-fndr-context-budget-CASCADE-REPORT.md`.

On 2026-07-17 (first restart after the drive-io merge) an FNDR @mention 400'd: "prompt is too long: 201524 tokens > 200000 maximum". VERIFY-FIRST via the Anthropic tokenizer: FNDR/HJRG were the only entities that wholesale-injected the full founder CLAUDE.md (`_FOUNDER_FULL_ENTITIES`), whose ~157K-token "Current State of the World" tail + tools (23,834) + the entity prompt (11,485) crossed 200K (block-2 static alone = 160,226 tok).

- **SLICE 2 (structural, durable fix):** `_FOUNDER_FULL_ENTITIES` emptied -- FNDR/HJRG now get the slim founder head + retrieval note like every other entity. Measured FNDR full request 199,463 -> 45,375 tokens. Safe because the founder doc is freshly + fully KB-indexed under FNDR (source=static_md, ~4,172 chunks, newest <1d) + co-scanned on every FNDR query, so portfolio-status questions retrieve the relevant current-state -- no answerable fact is lost, and the cached context block stops being invalidated by daily TOM edits.
- **SLICE 1 (belt):** a pre-send token-budget guard in claude_client estimates the request OFFLINE (char/3.0 -- a conservative upper bound; char/4 undercounts dense markdown ~30% and would let a 200K request through) and, over a configurable ceiling (`CLAUDE_MAX_INPUT_TOKENS`, default 185000), TRIMS in priority order: (A) reduce block-3 KB chunks lowest-relevance-first to a floor; (B) drop the wholesale founder CSotW from block-2 (KB-retrievable); (C) marker-independent truncation of the founder region as a last resort (survives a founder-doc heading rename on Drive). It NEVER trims the entity system prompt, tool schemas, per-entity known-answers/dynamic, or the runtime security/PHI/firewall/source-opacity/finance rules, and preserves cache_control on blocks 1+2. It degrades; it never raises a 400.
- **SLICE 3 (secondary):** the FNDR-cash / F3E-pipeline / F3-Pure dynamic-answers snapshots have served the yaml fallback ~58 days (manual seeds, no refresh writer). Confirmed NOT a prompt-size contributor (block ~3K tok; fallbacks honest). Added `nightly_health_check.check_dynamic_snapshots()` to surface the staleness daily + a defensive `_MAX_DYNAMIC_CHARS` cap. Rebuilding the 3 refreshers is a separate feature (flagged for Harrison).

**DOCTRINE (D-084):** (1) an offline token estimator for a hard input-ceiling guard must be a conservative UPPER bound on the real count -- calibrate the chars/token divisor to the DENSEST content class the request carries (Cora: KB chunks ~2.95, founder markdown ~3.06), never the char/4 default. (2) A per-request context assembly that inlines an ever-growing document (the founder brief) will silently drift into the input ceiling; prefer retrieval of the volatile tail + a stable static head, and back it with a degrade-don't-400 guard. (3) Never trim a security/PHI/firewall/source-opacity/finance instruction to meet a budget -- shed retrievable context (KB chunks, the KB-indexed CSotW) first, and if only protected content remains, attempt the send rather than sacrifice a guard.

## D-085 - Retire the stale manual dynamic-answer seeds (SLICE 3 close-out of D-084) (2026-07-18)

Branch `claude/retire-dynamic-answers` off `main`=`059381c`, PUSHED, FF-mergeable, awaiting merge. NO restart needed (context_loader reads the dynamic YAML live at the 5-min TTL + mtime fingerprint; nightly_health_check globs the seeds fresh each run). Suite 6901. Source of truth: `_shared/projects/cora/2026-07-18_fndr_cora-retire-dynamic-answers-CASCADE-REPORT.md`.

Closes D-084 SLICE 3 (which only SURFACED the staleness in the nightly health check and deferred the disposition). The four MANUAL `dynamic_answers` seeds -- FNDR/hjrg-cash-position, F3E/f3e-sales-pipeline, F3E/pure-launch, OSN/monthly-financials -- were hand-written once (2026-05-18/19), had NO auto-refresh writer ever built (every seed's "Phase 2 will auto-refresh from [sheet/HubSpot/sweep]" was never built), aged past their staleness thresholds ~2 weeks later, and have served the yaml `fallback` for ~60 days. Decision (Harrison, 2026-07-18): RETIRE, do not rebuild.

Why retire not rebuild (VERIFY-FIRST on `059381c`): (1) live tools ARE the answer path and supersede these pre-tool seeds -- `financial_get_cashflow` (TIER_1/FNDR, reads the Standing ACTUALS sheet), `f3e_hubspot_pipeline_summary` (F3E/FNDR, live HubSpot pipeline 2313722582), `osn_financial_pulse`/`financial_get_pulse`/`financial_get_close_pack` (OSN); (2) even at their freshest the finance seeds held no live figures (the FNDR cash snapshot's total literally read "pull from the canonical sheet"); (3) the stale snapshot content was factually wrong per the 2026-07-18 diagnosis (e.g. a retired HubSpot pipeline id + a since-passed launch date) -- serving "refreshed" static numbers would regress vs the safe fallback pointer; (4) a refresher would duplicate the live tools' fetch logic and re-introduce a serve-stale-numbers foot-gun the fallback design avoids.

- Deleted the 4 seed YAMLs + their gitignored local snapshots (`data/snapshots/{f3-pure,f3e-sales-pipeline,hjrg-cash-position,osn-monthly-financials}`). `data/snapshots/ads/manus-insights` is a SEPARATE ads tool (`ads_client._load_manus_snapshot`), untouched.
- KEPT the `dynamic_answers` MECHANISM intact + DORMANT: the module, the context_loader wiring (`_load_scoped_dynamic_answers`), the entity-scope firewall, and the entity subdir `.gitkeep` all remain -- a future properly-fed seed renders immediately, so the retirement is reversible. With zero seeds: `available_dynamic_entities()` returns the 5 empty entity dirs, all loads return "", context_loader appends no DYNAMIC section, and `check_dynamic_snapshots()` returns ok ("0 dynamic snapshot(s)") -- the 3 nightly WARNs (FNDR cash / F3E pipeline / F3 Pure) are gone; OSN monthly-financials (at its 1440h/60d threshold, about to WARN) was retired with them for consistency.
- Belt tests (`tests/test_dynamic_answers_retired.py`): the 3 live-tool answer paths stay defined + entity-exposed (FNDR/HJRG full; F3E cash+pipeline; OSN cash+pulse) + finance-gated (TIER_1/FNDR, refuses TIER_3); the mechanism stays reversible (a synthetic seed still renders through the firewall; OSN never sees F3E's seed). Plus a zero-seed `check_dynamic_snapshots -> ok` pin so the health check can't regress to WARN-on-empty.
- No resurrection risk: nothing re-materializes a seed from the `.resolved-gaps.jsonl` `action:"dynamic"` ledger line (that handler is gone; the readers -- gap_autofill / drive_materializer / ingest_digest_answers / backup_logs -- use the file only as a dedup/backup/materialize ledger).

**DOCTRINE (D-085):** (1) a static-context feature with no writer is rot -- it silently serves an ever-staler fallback; if a "Phase 2 will auto-refresh" never ships, RETIRE the seeds rather than let them masquerade as current. (2) When a live tool becomes the real answer path, retire the pre-tool static scaffolding it superseded -- do not keep both; the static copy drifts wrong and competes with the live fetch. (3) Retire the DATA (seeds), keep the MECHANISM dormant + reversible, so a future properly-fed snapshot is a drop-in, not a rebuild. D-051 review: 5 lenses (answer-path, empty-state, test-integrity, firewall, residue), 5 finders + per-finding verify, 0 findings -- clean.

## D-086 - Founder-OS KB cleanup: archive-move + KB purge, one folded pass (2026-07-21)

Branch `claude/founder-os-kb-archive-purge` off `main`=`34e3d63`, PUSHED, FF-mergeable, awaiting merge + Harrison-run `--apply` + ONE bot restart (the copa-bhrf exclusion in kb_exclusions/store is bot-loaded; the archive tool itself is script-side). Suite 6925 / 1 skipped. Source of truth: `_shared/projects/cora/2026-07-21_fndr_cora-kb-cleanup-archive-purge-CASCADE-REPORT.md`.

The single folded move+purge session the 2026-07-20/21 KB-cleanup initiative pointed at. `scripts/archive_founder_os_kb.py` (dry-run default; `--apply` Harrison-run after manifest review + db backup) moves ~569 approved stale `.md` (per the decision-complete pass-2 disposition list) into a mirrored reversible `_archive\<relpath>` tree + purges their KB chunks (static_md ~2341, drive-copy ~1557). NOTHING was moved/purged by this session -- Code builds + dry-runs + adversarially reviews; Harrison merges + applies.

- KB mechanics VERIFIED read-only: static_md source_id = exact BACKSLASH relpath; 3-table cascade = knowledge_chunks + knowledge_vec_bin (vec0) + knowledge_vec_f32 keyed by chunk_id (legacy knowledge_vec dropped); `_shared\projects\cora\**` = 0 chunks (KB-excluded -> those archives are a pure Drive move, no purge).
- Dual-key hazard handled: drive_sweep/drive_asset copies key the filename in `title`; generic scaffolding names collide portfolio-wide (00-README.md -> 35 file-ids). The drive-copy purge is SELF-GUARDED: a title is purged only if it maps to <= 2 Drive file-ids (distinctive dated names = 1); scaffolding basenames are refused + logged.
- copa-bhrf (disposition §2c): the LEX NDA'd M&A-diligence folder. Archive only the LOOSE duplicate `_notes\2026-05-15-fireflies-context-doc.md`; keep the canonical `LBHS - COPA Research Project\_notes\` copy IN PLACE; purge ALL 45 copa static chunks (whole-folder GLOB) + the 2 copa `.md` drive titles; permanent re-ingest exclusion via `is_copa_bhrf_path` (store Step-0 chokepoint + incremental_sync belt) + the folder-id `112C7ljGRI5VO_ic66fVGQk4kf6IC40HQ` in `KB_EXCLUDED_FOLDER_IDS` (drive_sweep). The LBHS-COPA / Voyager Fireflies MEETING transcripts (under `_shared\meetings\`, FNDR-scoped, broadly retrievable) are OUT of the authorized project-folder scope -- flagged to Harrison, NOT purged.
- Safety layers (over-inclusion is the cardinal sin): a HOLD hard-guard ABORTS (exit 2) on any watchtower/oneamerica/capital-raise/travel-points path (copa allowed only for the one loose-dup); a KEEP-as-class filter applies to globs AND explicit entries (only a 3-path CLASS_EXCEPTIONS allowlist bypasses); the human-reviewed manifest before `--apply` is the last line of defense. §1g was re-scoped conservatively to the disposition's NAMED one-time-prompt categories -- ~15 non-named scratch files KEPT + documented for optional one-word expansion.
- D-051 review (6 lenses / 11 agents, find -> adversarially verify): 2 CONFIRMED apply-robustness defects FIXED -- (a) the purge set was derived from a LIVE-TREE glob, so a purge-only second phase / resume re-globbed the already-moved tree and silently purged nothing for the glob clusters -> the manifest now PERSISTS the resolved purge chunk_ids and a new `--from-manifest` reads them (no re-glob), with a loud warning if a selection runs against an already-moved tree; (b) the manifest was written after the move with no try/finally -> now written BEFORE any mutation and re-written in a `finally`, and per-file move errors are soft so one Windows/Drive lock cannot strand the batch. Plus a NEEDS_HARRISON fix implemented (each purged drive title records its file-ids + per-file chunk counts + entity in the manifest, so every deleted Drive file is reviewable). 2 false-alarms refuted. NONE was over-inclusion -- the cardinal sin held.

**DOCTRINE (D-086):** (1) a path-keyed KB purge MUST pair with an ingest exclusion or the nightly sweep re-adds it -- and the exclusion must cover EVERY ingest path (static_md incremental + a full migrate_static_md rebuild, both via the store Step-0 chokepoint; drive_sweep via the folder-id in KB_EXCLUDED_FOLDER_IDS). (2) Never LIKE a path -- `_` is a LIKE wildcard and `\` a separator; use GLOB or exact IN() only. (3) The drive-copy leg is keyed by a bare filename `title` that collides portfolio-wide for scaffolding names -> self-guard the purge on distinct-file-id count, never trust a bare title. (4) A destructive one-pass tool's manifest is its reversibility rail: write it BEFORE mutating, persist the RESOLVED selection (not a re-globbable query, which goes empty once the tree is moved), and make it human-reviewable (list every deleted file-id). (5) Translating a human disposition into globs, over-inclusion is the cardinal sin -- the KEEP-as-class filter must bind explicit lists too (an explicit path is not a license to skip guards), and an ambiguous "whole-folder" reading should be re-scoped DOWN to named categories, documenting what was conservatively kept.

## D-087 - KB-staleness hygiene LOOP (supersede -> archive -> purge) + graduated-trust auto-write flip (2026-07-21)

Branch `claude/kb-hygiene-loop` off `main`=`1ce69d0`, test-gated per slice (suite 6994). Turns the one-time D-086 cleanup into a STANDING repeatable loop so create-new-dated-doc supersession never accumulates stale, retrievable doctrine, AND relaxes D-011 for reversible knowledge writes (Harrison directive 2026-07-21). Seven slices on one branch; the KB-hygiene half (A-E) is script-side (no restart); the auto-write flip (F) is bot-loaded + DEFAULT-OFF.

VERIFY-FIRST reconciliation (against live code, 9-subsystem parallel map): confirmed the replace-on-conflict-by-(source,source_id) staleness mechanism only handles the SAME source_id (superseded/renamed/deleted files persist -- the exact gap); static_md source_id = exact backslash relpath; the "3-table cascade" is an application-level manual delete (no SQL FK). Corrected the prompt where it drifted: `_FOUNDERS_OS_SKIP_FOLDERS` is drive_sweep's, not the static sync's (the sync skips `_archive` via a substring check); kb_exclusions guards 5 folder-ids (adds travel-points + a OneAmerica-Handoff dup), copa-bhrf is a SEPARATE segment set; the graduated-trust flip lives in the CONSUMER run_knowledge_review (the shadow module is pure instrumentation); and the shadow produced ZERO Tier-0/1 track record (19/19 -> Tier-2) -- so the flip rests on conservative tier scoping + reversibility, NOT a shadow verdict.

- **Refactor (A):** the D-086 move/purge/manifest engine lifted into a config-parameterized `src/cora/kb_archive.py` (`ArchiveConfig`, `build_move_manifest`, `select_static_purge`/`select_drive_purge`, `delete_chunks`, `plan_moves`/`execute_moves`, `revert`, `write_manifest`); `archive_founder_os_kb.py` becomes a thin config+CLI over it (its 25 tests pass unchanged). SQL byte-identical.
- **Banner (B):** `<!-- KB-STATUS: SUPERSEDED <date> by <ref> [-- <reason>] -->` (greppable, invisible in rendered md). `scripts/kb_hygiene_sweep.py --marked` walks the tree (mirroring the sync's skips), archives+purges banner'd files, WARNS (never archives) a banner on a KEEP-as-class file, and REFUSES (never touches) a banner in a HOLD/confidential path (watchtower/oneamerica/capital-raise/travel-points + copa-bhrf + the kb_exclusions predicates, union, fail-closed).
- **Proactive (C):** `--proactive` PROPOSE-ONLY detectors (near-dupe centroid-cosine reusing knowledge_vec_f32 vectors; TTL `_notes` one-offs keeping the latest RESUME-PROMPT/project; resolved decisions-pending). Never moves/purges -- Harrison banners the real ones.
- **Retention (D):** `--gc` hard-deletes `_archive` files older than `--purge-after-days` (180), clamped never to delete inside `--restore-days` (30) and resolved-path-contained under `_archive` (a `..` escape is refused). Reads ONLY this loop's `kb-hygiene-manifest-*` (never the D-086 manifest). `--from-manifest` resume; `--slack` DMs Harrison the report.
- **Wrappers (E):** monthly `cowork-cora-kb-hygiene` task (`--marked --apply --proactive --gc --slack`, user-level) + elevated `run-kb-hygiene-apply.ps1` (disable watchdog -> stop Cora -> backup db -> `--marked --apply --allow-large` -> optional reclaim -> restart -> re-enable watchdog).
- **AUTO-APPLY escalation (7A):** the monthly task auto-applies SMALL marked sweeps LIVE (WAL DELETE, no VACUUM, no Cora stop); a LARGE sweep (>500 chunks or >100 files) ESCALATES (exit 3) instead and DMs Harrison to run the stopped-window wrapper. Tiered restorable retention (30d easy-revert / ~180d GC).

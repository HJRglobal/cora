...
## D-079 - AI-Visibility 4th brand: Harrison Rogers (hjr) at measurement parity + person-brand AIO gate (2026-07-14)

Branch `claude/ai-visibility-hjr-brand` off `main`=`8509013`. PUSHED, NOT merged (Harrison FF). 3 commits (`cbb70a0` basket+tests / `c19662b` card+tool / `d56e813` D-051 remediation). Suite base **6,641** -> **6,647 passed** / import smoke OK. Opus 4.8 @ xhigh + ultracode. NOTE: D-079 is provisional -- D-078 is claimed by two concurrent branches (`mega-smoke-remediation`, `dashboard-read-layer`); Cowork assigns final numbers at promotion. Source of truth + full record: `_shared/projects/cora/2026-07-14_fndr_cora-ai-visibility-hjr-brand-CASCADE-REPORT.md`.
...
**DOCTRINE:**
1. Making a MEASUREMENT instrument's target a PERSON (not a product) changes the disambiguation surface: a namesake can pollute any name-matched signal. Aggregate third-party signals we cannot re-judge per-answer (the Otterly AIO leg) must be gated out of a person's scored headline; the direct, per-answer-judged engines carry the score. Keep it reversible + data-driven (`_PERSON_BRANDS`).

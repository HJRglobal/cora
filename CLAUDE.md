...
| 9/25 | Code #15 integrity + queue UX (`a8e9263e`) | API-token-shape redactor at every banking-belt renderer + before the session-capture prompt (tokens FIRST, LEX scrub included); ONE combined KB purge (dry-run INTENT, LARGE apply only in the stop window); `_archive` + personal-finances pins, desktop.ini + `_runs` belts; RIDER B holds proposal + weekly NO-HASH hygiene instrument (registration pending); gap executor = real project + owner or visible refusal, two-tier dedup; `cora_delegated_jobs` requester (founder-local); carve-out breach = no-record only; Monday-menu rows re-render; kickoff files carry a STATUS line + title H1, truncation marked; code-queue kickoffs off Bolt's listener pool (per-test drained in conftest); 4 worktree-red tests hermetic; its ONE restart done 9/25 07:06 AZ (pid 3844 in `logs/cora-instances.jsonl`) | `2026-09-24_fndr_cora-code-15-integrity-queue-ux-CASCADE-REPORT.md` |
| PUSHED | Code #16 new capability lanes (`claude/code-16-new-capability-lanes-2026-09-21`) | Slack inactive-channel archive lane born T0: monthly propose-only card in Harrison's DM (Section A clean / Section B per-row), per-row + card-level agree taps RECORDED as the T1 evidence, the T1 executor dark behind `CORA_CHANNEL_ARCHIVE=act` AND the live registry tier read at stage, tap and pre-notice (D-326 in code), fsync'd intent ledger + `check_channel_archive` in the 08:45 health check; travel shortlist T0 = its own web-only call built from parsed fields (`web_search_20250305` only, allowlist belt `assert_request_clean`, no booking/loyalty/PII by construction) + a `travel_lane` withhold on the normal Q&A web path; two T0 ladder rows + a validated `cap` key + the `channel_archive_mode` acting probe; 7.5 staged; needs ONE restart, then register the monthly task | `2026-09-25_fndr_cora-code-16-new-capability-lanes-CASCADE-REPORT.md` |

...
### [PUSHED] 2026-08-01 Filer/KB correctness bundle -- md5-verify + bot-reply tagging + gmail alias dedup + queue reconciliation (branch `claude/filer-kb-correctness-2026-07-31`, a31d8ac..487b968, suite 7704; awaiting FF-merge + ONE restart)

Post-D-095 loose ends, 5 slices + a 6-defect D-051 remediation (4-lens/14-agent review, 10 verified findings, 0 refuted). VERIFY-FIRST overturned two premises: (a) S2's named module `channel_sweep.py` already filters bots but never writes the KB -- the unfiltered ingest is `incremental_sync_slack.py` (45% of live slack chunks carry Cora speaker lines); (b) S2b's triplicate was NOT quoted-reply copies -- the same PHYSICAL Gmail mailbox is roster-enrolled under 2-3 alias addresses and `source_id` embeds the roster email, so replace-on-conflict could never fold the copies (~120K pure alias-dup rows, ~28% of the gmail partition).

- **S1 (`a31d8ac`, BOT-LOADED)**: `upload_file` layer-1 name-dedup now md5-verifies when `content_md5` is supplied (all candidates scanned, mismatch -> uniquified upload `-2/-3`); returns `(file_id, web_link, final_name)`; `process_email` keeps ledger+digest truthful to the Drive name. finance_receipts passes no content_md5 -> byte-identical legacy semantics. Closes the D-095 flagged follow-up (cq-349a17012ddf).
- **S2 (`ab74e40`, ingest script-side + context_loader BOT-LOADED)**: slack ingest tags per-chunk `bot_authored` (all-app chunk) + `has_cora_reply` (any Cora msg; auth.test id, U0B44MDGC5R fallback); join/leave/topic skipped. gap_autofill + friction_mining + reconciliation_engine exclude bot_authored chunks; `_format_kb_chunks` labels Cora's own replies "not canon". STAGED: `scripts/retro_tag_bot_slack_chunks.py --apply` (1,996 rows, dry-run verified). (cq-8d16969e85fb)
- **S2b (`50b1a61` + `487b968`, script-side)**: `gmail_threaded_sweep` collapses alias accounts to one physical-mailbox sweep (getProfile canonical, num_retries=2, fail-soft; watermark = max over alias keys AND mirrored back to alias keys each night); quote-strip with forward-marker exemption; duplicate-delivery fingerprint keyed sender+attachments+body; `--accounts` filter runs AFTER the collapse. STAGED: `scripts/purge_gmail_alias_dup_chunks.py --apply` (120,068 rows; groups on gmail-api message_id + entity + sub_entity + content-hash -- same-mailbox-proof, no cross-person folds; --apply opens via schema.connect for the vec0 extension). kb_archive + prune_kb_retention cascades now include knowledge_vec_bin_v2 (candidate-list, NEVER bare LIKE discovery -- vec0 shadow tables).
- **S3 (`022720d`, data-only, EXECUTED)**: 14 D-095 seeds PROPOSED->SHIPPED via `process_queue_action` (13 bundle + cq-7fb82054ee4a premise-overturned-residual-shipped; cq-9845f2effb8d was misfiled in the note's verify bucket -- it WAS in the bundle); 5 verified-not-in-bundle left open. Loop step 7.5 added (RECONCILE the queue at merge). Backlog verified reconciled via cora_code_queue.
- **S4 (`9535d60`, code_queue BOT-LOADED but script-invoked, EXECUTED)**: `apply_prompt_rehome` grouped by source file (the 7/28 copy-then-unlink deleted a 6-row shared bundle prompt after row 1); `plan_prompt_rehome` self-heals (backfill-only when dst exists) + STAGED-only guard (a staged backfill on a terminal row would RESURRECT it -- the cq-dad80c0011c9 trap, its inert stale path deliberately left). Live: 5/5 rows repointed to the G: lex-ar-audit path.
- **Both sweeps (`487b968`)**: ALL chunks of one message/thread now share the BARE source_id -- replace-on-conflict dedups keys batch-wide, so re-ingest replaces the family atomically regardless of chunk-count changes (the ':chunkN'-when-multi scheme orphaned rows when quote-strip shrank a body or a slack thread GREW -- the slack half was a pre-existing latent bug).

**HARRISON EXECUTION LIST**: (1) FF-merge -> push main; (2) ONE elevated restart (S1 drive_connector + S2 retrieval labels + S4 code_queue are bot-loaded); (3) `scripts/reconcile_code_queue_filer_kb_2026-08-01.py --apply` (step 7.5 for this bundle's 2 seeds); (4) KB backup then `scripts/purge_gmail_alias_dup_chunks.py --apply`; (5) `scripts/retro_tag_bot_slack_chunks.py --apply`; (6) smokes -- two distinct same-named attachments in separate emails -> two Drive files; the wholesale-pure F3E query -> one gmail chunk post-purge.

Doctrines (proposed D-096, cascade pending): population/identity dedup belongs at the PHYSICAL-resource layer (getProfile canonical mailbox), not roster-entry discipline; a purge cascade uses a NAMED candidate list existence-checked (bare LIKE 'knowledge_vec%' matches vec0 shadow tables -- deleting corrupts the index); vec0 DELETEs require schema.connect (plain sqlite3 tests pass vacuously on plain tables); chunk-family replacement must key on the STABLE id (bare source_id), never a count-dependent suffix; appending a 'staged' event to a terminal queue row resurrects it (fold is last-write-wins).

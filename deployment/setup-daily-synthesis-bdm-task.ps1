# Setup Windows Scheduled Task: Cora daily BDM synthesis, 06:52 AZ.
#
...
# BDM synthesis to #bdm-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:52 AZ is a free
...
#     .\deployment\setup-daily-synthesis-bdm-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (BDM)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (BDM)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (BDM)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity bdm" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Big D Media operational synthesis (cash, pipeline, deadlines) to #bdm-leadership; client-confidential" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity bdm --dry-run"

# Setup Windows Scheduled Task: Cora daily LEX synthesis, 06:39 AZ.
#
...
# LEX synthesis to #lex-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:39 AZ is a free
...
#     .\deployment\setup-daily-synthesis-lex-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (LEX)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (LEX)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (LEX)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity lex" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "Lexington Services AGGREGATE operational synthesis (cash, counts-only deadlines, decisions) to #lex-leadership; PHI-safe, no client detail" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity lex --dry-run"

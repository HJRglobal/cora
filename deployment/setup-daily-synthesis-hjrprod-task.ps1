# Setup Windows Scheduled Task: Cora daily HJRPROD synthesis, 06:56 AZ.
#
...
# HJRPROD synthesis to #hjrprod-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:56 AZ is a free
...
#     .\deployment\setup-daily-synthesis-hjrprod-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (HJRPROD)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (HJRPROD)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (HJRPROD)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity hjrprod" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "HJR Productions operational synthesis (cash, deadlines, decisions) to #hjrprod-leadership; expect brief posts" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity hjrprod --dry-run"

# Setup Windows Scheduled Task: Cora daily UFL synthesis, 06:54 AZ.
#
...
# UFL synthesis to #ufl-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:54 AZ is a free
...
#     .\deployment\setup-daily-synthesis-ufl-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (UFL)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (UFL)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (UFL)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity ufl" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "United Fight League operational synthesis to #ufl-leadership; entity PAUSED -- expect quiet-day posts (Harrison may leave this task unregistered)" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity ufl --dry-run"

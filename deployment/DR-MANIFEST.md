...
_Probe baseline 2026-09-23 AZ on `H-REMOTE` by `scripts/dr_manifest_probes.py` (live root `C:\Users\Harri\code\cora`): **PASS 19 / FAIL 0 / MANUAL 3 / UNMEASURED 1 / INFO 1** of 24 items. Machine-readable twin: `deployment/manifest/dr-manifest.json`. Regenerate with `--update-docs`; do not hand-edit._

| # | Item | Source of truth | Restore step | Probe (this host) | Owner | RTO |
|---|---|---|---|---|---|---|
| D01 | Repo + branch (`main`) + remote | GitHub HJRglobal/cora; local `C:\Users\Harri\code\cora` | `git clone <origin> C:\Users\Harri\code\cora`; checkout `main` at the recorded hash | **MANUAL** -- restore hash: origin/main f65b7fc; live checkout HEAD 6f65b37 on claude/deposco-push-write-path-2026-09-22; origin set -- the live checkout is NOT on main (a Code session holds it); the restore target is origin/main, not this HEAD | Harrison | ~5 min |
...
| D08 | Live bot health (heartbeat + instances ledger) | `data/health/heartbeat.txt` (60 s) + `logs/cora-instances.jsonl` | start the service task; heartbeat must advance within 2 min | PASS -- heartbeat -4 s old; pid 5384 started 2026-09-20T05:44:15Z | Cora-script | ~2 min |
...
| D22 | Task XML restore set (`deployment/manifest/tasks/<slug>.xml`, committed) + local `deployment/task-backups/<date>` exports | the generator exports every task's XML on each run (committed with the manifest); `rewrap-tasks-hidden.ps1` also writes local, gitignored dated exports | the committed XML set IS the restore form (D13); the dated local exports are a convenience that does not survive a bare-metal loss | PASS -- 96 committed task XML files under deployment/manifest/tasks; covers every live task; local dated exports (gitignored convenience): ['2026-09-02', '2026-09-03'] | Cora-script | n/a |

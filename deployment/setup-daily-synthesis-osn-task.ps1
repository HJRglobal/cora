# Setup Windows Scheduled Task: Cora daily OSN synthesis, 06:37 AZ.
#
...
# OSN synthesis to #osn-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:37 AZ is a free
...
#     .\deployment\setup-daily-synthesis-osn-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (OSN)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (OSN)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (OSN)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity osn" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "One Stop Nutrition operational synthesis (cash, pipeline, deadlines) aggregate across stores to #osn-leadership" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity osn --dry-run"

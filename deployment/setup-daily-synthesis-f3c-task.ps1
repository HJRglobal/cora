# Setup Windows Scheduled Task: Cora daily F3C synthesis, 06:58 AZ.
#
...
# F3C synthesis to #f3c-leadership.
#
...
#
# Slot check (2026-07-07, verified against the live registry): 06:58 AZ is a free
...
#     .\deployment\setup-daily-synthesis-f3c-task.ps1
#
# To remove:
#     Unregister-ScheduledTask -TaskName 'Cora - Daily Synthesis (F3C)' -Confirm:$false

$ErrorActionPreference = "Stop"

$RepoRoot   = "C:\Users\Harri\code\cora"
$PythonExe  = "C:\Users\Harri\code\cora\.venv\Scripts\python.exe"
$TaskName   = "Cora - Daily Synthesis (F3C)"
...
$action = New-WrappedTaskAction -TaskName 'Cora - Daily Synthesis (F3C)' `
    -Execute $PythonExe `
    -Argument "`"$ScriptPath`" --entity f3c" `
...
Register-ScheduledTask `
    -TaskName $TaskName `
    -Description "F3 Community operational synthesis (deadlines, decisions; cash tracked under F3 Energy) to #f3c-leadership; nonprofit -- expect brief posts" `
    -Action $action `
    -Trigger $trigger `
    -Principal $principal `
...
Write-Host "  & '$PythonExe' '$ScriptPath' --entity f3c --dry-run"

...
def test_windows_are_adjacent_30d():
    cur, prior = brief._windows(_TODAY)
    assert cur == ("2026-05-19", "2026-06-18")
    assert prior == ("2026-04-19", "2026-05-19")  # prior abuts current
...
def test_dtc_line_degrades():
    with patch.object(brief.shopify_client, "get_sales_pulse", side_effect=ShopifyConnectorError("no token")):
        assert brief._dtc_line() == "- *DTC (Shopify):* not available"
...
def test_paid_line_renders_with_delta_and_top_channel():
    cur = _polar(
        {"total_marketing_spend": 8000, "blended_roas": 3.5, "blended_net_sales": 28000},
        table=[
            {"custom_internal-default-channel-grouping": "Meta", "total_marketing_spend": 6000},
            {"custom_internal-default-channel-grouping": "TikTok", "total_marketing_spend": 2000},
            {"custom_internal-default-channel-grouping": "Zero", "total_marketing_spend": 0},
        ],
    )
...
def test_subs_line_polar_not_connected():
    with patch.object(brief, "_polar_report", side_effect=PolarConnectorError("x")):
        assert brief._subs_line(_TODAY) == "- *Subs (ReCharge):* Polar not connected yet"
...
def test_retail_line_degrades():
    with patch.object(brief.hubspot_client, "get_deals_by_pipeline",
        ...
        assert brief._retail_line() == "- *Retail (HubSpot):* not available"
...
def test_run_total_degradation_still_builds():
    ...
    assert "Polar not connected yet" in out
